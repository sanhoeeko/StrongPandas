...
class Variable:
    def __init__(self, name):
        """
        if the name is not specific, use void string '' as the name
        """
        self.name = name
        self.arr = None
        self.other_info = []
    ...
    def astype(self, strongPandasType):
        return strongPandasType(self)

    # operator is for constructing calculation graph
    def __add__(self, x):
        if not issubclass(type(x), Variable):
            x = constant(x)
    ...
    def draw(self, *ys: 'Variable', style='auto'):
        """
        draw a diagram, as x value.
        other style:
            dense: if x value has a great amount
        """
        if style == 'auto':
            for y in ys:
    ...
    def make_info_dict(self, separator=': '):
        if self.other_info is not None:
            for i in range(len(self.other_info)):
                lst = list(map(lambda x: x.split(separator), self.other_info[i]))
